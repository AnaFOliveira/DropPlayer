...
##               
    ...
    def playlists(credentials,editor):
        ...
        while shown:
            ...
            wish=m.menuPlaylist()
            #devia se verifyP
            if wish=='a': #Ver detalhes e músicas da playlist
                pub=input('Pretende saber os detalhes de uma música pública ou privada? (pub/priv)')
                if pub=='pub':
                    username=input('Username do criador da playlist: ')
                    nome=m.selectPlaylist()
                    pl.getList(username,nome)
                else:
                    nome=m.selectPlaylist()
                    pl.getList(credentials[0],nome)
            ...
            if wish=='c':
                nome = m.selectPlaylist()
                idM = m.addMusicToPlaylist()
                pl.updateList(credentials[0],nome,idM)
            
            if wish=='d':
                nome=m.selectPlaylist()
                posicao=m.getSong()
                idM=pl.deleteMusicaP( posicao, nome,credentials[0])
